...
export class PDFSigner {
  ...
  private static debugBuffer(buffer: Buffer, label: string) {
    ...
    console.log('Hash:', crypto.createHash('sha256').update(buffer).digest('hex'));
  }
  ...
  private static findSignatureLocations(pdfBuffer: Buffer): { 
    ...
    if (byteRangePos === -1) throw new Error('ByteRange not found in signature dictionary');
    if (contentsPos === -1) throw new Error('Contents not found in signature dictionary');

    const contentsHexStart = searchRange.indexOf('<', contentsPos);
    if (contentsHexStart === -1) throw new Error('Contents opening delimiter not found');
  ...
  private static getContentToSign(pdfBuffer: Buffer, byteRanges: number[]): Buffer {
    const firstPart = pdfBuffer.slice(byteRanges[0], byteRanges[1]);
    const secondPart = pdfBuffer.slice(byteRanges[2], byteRanges[2] + byteRanges[3]);
  ...
  public static async signPDF(
    ...
    try {
      ...
      const byteRangeString = ` [${byteRanges.map(n => 
        n.toString().padStart(10, '0')).join(' ')}]`;
      ...
      const signatureHex = signature.toString('hex').padEnd(this.SIGNATURE_LENGTH, '0');
      finalPdf.write(signatureHex, contentsPlaceholder);

      // Verify immediately
      const publicKey = crypto.createPublicKey(privateKey);
      const isValid = await this.verifySignature(finalPdf, 
  ...
  public static async verifySignature(
    ...
    try {
      ...
      const pdfString = pdfBuffer.toString('binary');
      const byteRangeStr = pdfString.slice(byteRangePlaceholder, byteRangePlaceholder + 50);
      const byteRangeMatch = byteRangeStr.match(/\[([\d\s]+)\]/);
      
      if (!byteRangeMatch) throw new Error('Could not extract ByteRange values');
